...
用于创建并返回一个与特定窗口关联的2D渲染器。渲染器可以用来绘制纹理、线条、矩形等图形元素。渲染器的具体实现可能是基于硬件加速或软件模拟的。

<font color=orange>输入</font>

`window` ：目标窗口的指针，渲染器将在该窗口上进行绘制。必须是一个有效的窗口对象。
`index` ：渲染驱动程序的索引，若为-1，SDL会自动选择第一个可用的驱动程序。若指定具体索引，则使用该索引对应的驱动程序（如Direct3D、OpenGL）。
`flags` ：渲染器的标志，用于控制渲染器的行为。**可以是以下值的组合**：
...
> 创建纹理时，传入的渲染器只是指定了纹理的初始状态，并为该纹理选择合适的像素格式和硬件支持。然而，**纹理本身并不会直接依赖于渲染器的生命周期。**
